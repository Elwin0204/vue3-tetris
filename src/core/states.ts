  ...
  const getLine = (min: number, max: number) => {
    ...
    for (let i = 0, len = 10 - count; i < len; i++) {
      ...
      const index = parseInt(
        ((line.length + 1) * Math.random()).toString(),
        10
      );
...
interface IStates {
  ...
  nextAround: (matrix: number[][], stopDownTrigger?: () => void) => void;
  focus: (isFocus: boolean) => void;
  pause: (isFocus: boolean) => void;
  clearLines: (matrix: number[][], lines: number[]) => void;
  overStart: () => void;
  overEnd: () => void;
...
const states: IStates = {
  // Automatically drop setTimeout variable
  ...
  auto: (timeout = 0) => {
    ...
    states.fallInterval = setTimeout(
      fall,
      out === undefined || out === 0 ? speeds[state.speedRun - 1] : out
    );
  },
  // 一个方块结束, 触发下一个
  ...
  //  Write the score and judge whether to create the highest score
